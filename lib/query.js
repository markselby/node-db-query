...
p.execute = function() {
  ...
  pg.connect(this.__proto__.pool, function(err, client, done) {
    ...
    var q = this.query();
    (this.name ?
      client.query({ name: this._name, text: q.text, values: q.values }, this.end.bind(this)) :
      client.query(q.text, q.values, this.end.bind(this))
    )
