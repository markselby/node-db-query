...
p.parameterize = function(params) {
  ...
  if(params.page) {
    this.offset((params.page - 1) * parseInt(params.limit));
  }
}
...
p.select = function(fields, options) {
...
p.highlight = function(fields, options) {
  options = options || {};
  opt = 'MaxWords=35, MinWords=10, ShortWord=3, HighlightAll=FALSE, MaxFragments=10, FragmentDelimiter=" ... "';
...
p.execute = function(options, cb) {
  ...
  if(options.rows) {
    this.todo++;
    pg_pool.query(this.query())
      .on('error', console.error)
      .on('error', function() { this.todo--; }.bind(this))
      .on('row', this.row.bind(this))
      .on('end', this.end.bind(this));
  };
  if(options.count) {
    this.todo++;
    if(options.count instanceof sql) {
      options.count.execute({ rows: false, count: true }, this.count_cb.bind(this));
    }
    else {
      pg_pool.query(this.count_query())
        .on('error', console.error)
        .on('error', function() { this.todo--; }.bind(this))
        .on('row', this.count_row.bind(this))
        .on('end', this.end.bind(this));
    }
  }
}
...
// If we call a count query, this gets called when we get the row back
p.count_row = function(row) {
  this.result.count = row.count;
}

// Count can be another query, so it will call this when done
p.count_cb = function(result) {
...
p.end = function() {
  if(!(--this.todo)) this.cb(this.result);
}
