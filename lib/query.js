...
p.execute = function(cb) {
  this.rows = [];
  this.cb = cb;
  var self = this;

  console.log('Pool : ' + inspect(this.__proto__.pool));

  pg.connect(this.__proto__.pool, function(err, client, done) {
    if (err) {
      throw err;
    }
    this.done = done;
    ...
    console.log({ name: this._name, text: q.text, values: q.values });

    (this.name ?
      client.query({ name: this._name, text: q.text, values: q.values }, this.result.bind(this)) :
      client.query(q.text, q.values, this.result.bind(this))
    )
...
p.name = function(queryName) {
  this._name = queryName;
};
...
function Query() {
  ...
  for (var key in keys) {
    Q.prototype[keys[key]] = p[keys[key]];
  }

  return Q;
}

// Main usage. Connections are automatically placed back into the pool after execution.
var query = function(tables) {
  var q = new this.Query();
  if (tables) { q.from(tables); }
  return q;
}
...
module.exports = function(pool) {
  var q = Query(); //Query;
  q.prototype.pool = typeof pool == 'string' ? grunt.file.readYAML('config/database.yml')[pool] : pool;
  return { Query: q, named: named, query: query };
}
